...
Правила активации улья (улей — это люди, а не кадры) и стадия кластера: **`BACKEND_GROWTH_TZ.md` §G13** (контракт) и **`RN_GROWTH_TZ.md` §G13** (клиент). Этот документ описывает улей в его текущем виде и на §G13 только ссылается.  
Места заведений (кафе/бары), верификация партнёра и фото места из галереи: **`PLACE_PARTNER_TZ.md`** (§G14, §G15). Улей по-прежнему нельзя «создать» заведению; галерея разрешена только для витрины места, не для жал на карте.
...
Важно про тестовые данные: «3 жала в одной точке» от одного аккаунта дают улей только в текущей схеме, где порог считает кадры. По `BACKEND_GROWTH_TZ.md` §G13 активация переезжает на число участников (≥ 3 жала минимум от 2 авторов), поэтому проверять улей нужно с двух аккаунтов, а соло-кластер — это отдельное состояние («сота», `RN_GROWTH_TZ.md` §G13), а не улей.
...
| `hives`    | Ульи           | Только активные ульи, одиночные жала скрыты   | `hives[].stage` / `isActiveHive` (§G13) |
| `expiring` | Скоро исчезнет | Только жала на исходе TTL (§9.2), ульи скрыты | `Sting.createdAt`, `Sting.expiresAt`    |

`fresh` и `expiring` — про **отдельные жала**, поэтому ульи под ними скрываются: TTL фото внутри улья клиенту неизвестен без `GET /hives/:id`, и подмешивать улей в выборку по времени было бы ложью. Условие активности улья остаётся общим — предикат `isActiveHive` из `src/utils/hive.ts`, отдельной логики для фильтра не вводить. После §G13 предикат смотрит на стадию кластера, и соты (соло-кластеры) под фильтром «Ульи» не показываются — иначе фильтр «где сейчас люди» снова начнёт врать.
...
- [ ] `Ульи` — только активные ульи по `isActiveHive`, одиночных пинов и сот (§G13) нет
